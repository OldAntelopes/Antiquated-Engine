...
#else
	sprintf( szLocalFilenameRepathed, "%s\\%s", SysGetWritableDataFolderPath(), szLocalFilename );
#endif
	*pnCachedFileSize = 0;
...
eCacheCheckResult		nCacheFileRet;
	...
	sprintf( acTempFile, "%s\\%s", SysGetWritableDataFolderPath(), acLocalFileName );
...
//			PrintConsoleCR(acString, COL_WARNING );
	...
	char	acLocalFileNameRepathed[256];
		...
		sprintf(acLocalFileNameRepathed, "%s\\%s", SysGetWritableDataFolderPath(), acLocalFileName );
