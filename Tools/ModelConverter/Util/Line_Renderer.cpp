...
BOOL		msbLineRendererIsLocked = FALSE;
CUSTOMVERTEX* mspLineRendererVertices;
...
CUSTOMVERTEX* pVertices;
	...
	if ( msbLineRendererIsLocked == FALSE )
	{
	    if( FAILED( mpxLineVertexBuffer->Lock( 0, 0, (void**)&pVertices, D3DLOCK_DISCARD) ) )
		{
			return;
		}
		mspLineRendererVertices = pVertices;
		msbLineRendererIsLocked = TRUE;
	}
	else
	{
		pVertices = mspLineRendererVertices;
	}
...
//char	acString[256];
	...
	if ( msbLineRendererIsLocked == TRUE )
	{
		mpxLineVertexBuffer->Unlock();
		msbLineRendererIsLocked = FALSE;
	}
...
D3DXMATRIX	matWorld;
	...
	if ( nDrawHowMany > 0 )
	{
		...
		g_pd3dDevice->SetStreamSource( 0, mpxLineVertexBuffer, 0, sizeof(CUSTOMVERTEX) );

	    if ( g_pd3dDevice->DrawPrimitive( D3DPT_LINELIST, 0, nDrawHowMany ) != D3D_OK )
		{
#ifdef TOOL
...
void InitialiseLineRenderer( int nMaxVertices )
{
	...
	if ( mpxLineVertexBuffer == NULL )
	{
	    ...
	    if( FAILED( g_pd3dDevice->CreateVertexBuffer( mnMaxLineVertices * sizeof(CUSTOMVERTEX),
