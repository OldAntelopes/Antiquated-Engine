...
char	acString[256];
	...
	if ( pMaterialData )
	{
		szFilename = pMaterialData->GetFilename();
		if ( szFilename )
		{
		...
		for ( nLoop = 0; nLoop < nNumMaterials; nLoop++ )
		{
			if ( nLoop != mnMaterialBrowserCurrentAttrib )
			{
				...
				if ( pMaterialData )
				{
					...
					{
						pMaterialData->CloneTextureFromMaterial( NULL );
					}
				}
			}
		}
...
ModelMaterialData*		pMaterialData;
	...
	byte*	pbMem;
		...
		if ( pFile )
		{
			...
			pMaterialData->SetSourceData( pbMem, nFileSize );
			SystemFree( pbMem );
			pMaterialData->LoadTextureFilename( 0, acFilename );
		...
		case BN_CLICKED:
			...
			case IDC_CHANGE_DIFFUSE:
				...
				HWND store = (HWND)SysGetMainWindow();
					...
					acFilename[0] = 0;
					if ( SysGetOpenFilenameDialog( "All supported formats (*.dds,*.bmp,*.jpg,*.tga,*.png)\0*.dds;*.jpg;*.bmp;*.tga;*.png\0DirectDraw Surface(*.dds)\0*.dds\0Jpeg File(*.jpg)\0*.jpg\0Windows bitmap(*.bmp)\0*.bmp\0All Files(*.*)\0*.*\0", "Select Texture", acCurrentDir, 0, acFilename ) > 0 )
					{
