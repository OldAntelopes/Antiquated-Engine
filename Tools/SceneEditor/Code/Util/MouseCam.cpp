...
float	fMoveSpeed = mfMoveSpeed;
	...
	fScreenYDelta *= fMoveSpeed * -1.0f;
	fScreenXDelta *= fMoveSpeed;
...
{
float	fSpeed = mfRotSpeed;

	if ( SysCheckKeyState( KEY_SHIFT ) == TRUE )
	{
		fSpeed *= 3.0f;
	}
	mfViewAngleVert = mfOriginalViewAngleVert + (fTotalYDelta*fSpeed);
	if ( mfViewAngleVert > TwoPi )
	{ 
		mfViewAngleVert -= TwoPi;
	}
	...
	mfViewAngleHoriz = mfOriginalViewAngleHoriz + (fTotalXDelta*fSpeed);
	if ( mfViewAngleHoriz > TwoPi )
	{ 
		mfViewAngleHoriz -= TwoPi;
	}
...
POINTS points;
	...
	case 0x020A:		// WM_MOUSEWHEEL - for some reason i cant get it to include..
		...
		VECT	xMove;
			...
			VectScale( &xMove, &mxMouseCamView, ((fOffset * mfZoomSpeed)*0.5f));
			VectAdd( &mxMouseCamPos, &mxMouseCamPos, &xMove );
			MouseCamUpdateCameraMatrices();
		}
		break;
