...
BOOL	mbLandscapeCollisionInit = FALSE;
...
{
	// TODO - Optimise this.. could be completely precalculated...
...
ENGINEPLANE		mxLandscapeCollisionFloorPlane;
ENGINEPLANE		mxLandscapeCollisionCeilingPlane;

void	LandscapeCollisionInitialise( void )
{
ENGINEPLANE		xPlane;
VECT	xFloorPos1 = { -1000.0f, -1000.0f, 0.0f };
VECT	xFloorPos2 = { 1000.0f, -1000.0f, 0.0f };
VECT	xFloorPos3 = { -1000.0f, 1000.0f, 0.0f };
VECT	xCeilingPos1 = { -1000.0f, -1000.0f, 260.0f };
VECT	xCeilingPos2 = { 1000.0f, -1000.0f, 260.0f };
VECT	xCeilingPos3 = { -1000.0f, 1000.0f, 260.0f };

	EnginePlaneFromPoints( &mxLandscapeCollisionFloorPlane, &xFloorPos1, &xFloorPos2, &xFloorPos3 );
	EnginePlaneFromPoints( &mxLandscapeCollisionCeilingPlane, &xCeilingPos1, &xCeilingPos2, &xCeilingPos3 );

	mbLandscapeCollisionInit = TRUE;
}
...
VECT	xRayDir = *pxRayDir;
	...
	if ( mbLandscapeCollisionInit == FALSE )
	{
		LandscapeCollisionInitialise();
	}
	...
	if ( EnginePlaneIntersectLine( pxIntersectPoint, &mxLandscapeCollisionFloorPlane, pxRayStart, &xRayEnd ) == TRUE )
	{
	int		nMapX1, nMapX2;
	int		nMapY1, nMapY2;
	int		nLoopX, nLoopY;
	...
	float	fClosestCollision = VectGetLength( pxRayDir ) - 0.01f;
	BOOL	bColl = FALSE;
	VECT	xRayStart;

		if ( pxRayStart->z > 260.0f )
		{
			if ( EnginePlaneIntersectLine( &xRayStart, &mxLandscapeCollisionCeilingPlane, pxRayStart, &xRayEnd ) == TRUE )
			{
				fClosestCollision -= VectDist( &xRayStart, pxRayStart );
				pxRayStart = &xRayStart;
			}
		}
