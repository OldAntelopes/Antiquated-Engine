...
BOOL JpegFile::RGBToJpegFile(char* fileName, 
							...
							UINT height,
							BOOL color, 
							int quality, BOOL mirror, BOOL bFlip )
{
BYTE	bMirrorImage[1024*3];
int		Loop;
int		nMirrorPos;
...
/* Now use the library's routine to set default compression parameters.
  ...
  while (cinfo.next_scanline < cinfo.image_height)
  {
	...
	if (color) 
	{
		if ( bFlip )
		{
			outRow = dataBuf + ( ((cinfo.image_height-1)*widthPix*3) - (cinfo.next_scanline * widthPix * 3));
		}
		else
		{
	...
	{
	LPBYTE mirrorRow;
		nMirrorPos = (widthPix*3)-3;
		for ( Loop = 0; Loop < (int)widthPix; Loop++ )
		{
			bMirrorImage[ (Loop*3) ] = outRow[ nMirrorPos ];
			bMirrorImage[ (Loop*3)+1 ] = outRow[ nMirrorPos+1 ];
			bMirrorImage[ (Loop*3)+2 ] = outRow[ nMirrorPos+2 ];
			nMirrorPos -= 3;
		}
		mirrorRow = &bMirrorImage[0];
		(void) jpeg_write_scanlines(&cinfo, &mirrorRow, 1);
	}
	else
	{
