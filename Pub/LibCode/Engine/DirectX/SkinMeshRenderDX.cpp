...
#include "..\LibCode\Interface\DirectX\Window\D3DUtil.h"
#include "..\LibCode\Interface\DirectX\Window\DXUtil.h"
#include "..\LibCode\Interface\DirectX\Window\D3DRes.h"
...
HRESULT DrawMeshContainer(SMeshContainer *pmcMesh)
{
    ...
    if (pmcMesh->m_pSkinMesh)
    {
        ...
        if (m_method == D3DNONINDEXED)
        {
            ...
            for (ipattr = 0; ipattr < pmcMesh->cpattr; ipattr++)
            {
                ...
                if (m_d3dCaps.MaxVertexBlendMatrices >= numBlend + 1)
                {
                    for (DWORD i = 0; i < pmcMesh->m_maxFaceInfl; ++i)
                    {
                        ...
                        if (matid != UINT_MAX)
                        {
                            mpEngineDevice->SetTransform(D3DTS_WORLDMATRIX(i), pmcMesh->m_pBoneMatrix[matid]);
                            mpEngineDevice->MultiplyTransform(D3DTS_WORLDMATRIX(i), &pmcMesh->m_pBoneOffsetMat[matid]);
                        }
                    }

                    mpEngineDevice->SetRenderState(D3DRS_VERTEXBLEND, numBlend);

                    if ((AttribIdPrev != pBoneComb[ipattr].AttribId) || (AttribIdPrev == UNUSED32))
                    {
                        EngineSetMaterial(&(pmcMesh->rgMaterials[pBoneComb[ipattr].AttribId]));
                        mpEngineDevice->SetTexture(0, pmcMesh->pTextures[pBoneComb[ipattr].AttribId]);
            ...
            {
                AttribIdPrev = UNUSED32; 
                mpEngineDevice->SetRenderState(D3DRS_SOFTWAREVERTEXPROCESSING, TRUE);
                ...
                mpEngineDevice->SetRenderState(D3DRS_SOFTWAREVERTEXPROCESSING, FALSE);
            }
            mpEngineDevice->SetRenderState(D3DRS_VERTEXBLEND, 0);
        }
        else if (m_method == D3DINDEXEDVS)
        {
            ...
            hr = mpEngineDevice->SetStreamSource(0, pVB, D3DXGetFVFVertexSize(pmcMesh->pMesh->GetFVF()));
            if(FAILED(hr))
                return hr;
            hr = mpEngineDevice->SetIndices(pIB, 0);
            if(FAILED(hr))
                return hr;
            pVB->Release();
            pIB->Release();

            hr = mpEngineDevice->SetVertexShader(m_dwIndexedVertexShader[pmcMesh->m_maxFaceInfl - 1]);
            if(FAILED(hr))
                return hr;
...
//TODO!                        D3DXMatrixMultiplyTranspose(&mat, &mat, &m_mView);
                        mpEngineDevice->SetVertexShaderConstant(i*3 + 9, &mat, 3);
                    }
                }
                ...
                mpEngineDevice->SetVertexShaderConstant(8, &(pmcMesh->rgMaterials[pBoneComb[ipattr].AttribId].Diffuse), 1);
                mpEngineDevice->SetVertexShaderConstant(7, &ambEmm, 1);
                vConst.y = pmcMesh->rgMaterials[pBoneComb[ipattr].AttribId].Power;
                mpEngineDevice->SetVertexShaderConstant(0, &vConst, 1);

                mpEngineDevice->SetTexture(0, pmcMesh->pTextures[pBoneComb[ipattr].AttribId]);

                hr = mpEngineDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 
        ...
        else if (m_method == D3DINDEXED)
        {
            ...
            if (pmcMesh->m_maxFaceInfl == 1)
                mpEngineDevice->SetRenderState(D3DRS_VERTEXBLEND, D3DVBF_0WEIGHTS);
            else
                mpEngineDevice->SetRenderState(D3DRS_VERTEXBLEND, pmcMesh->m_maxFaceInfl - 1);
            if (pmcMesh->m_maxFaceInfl)
                mpEngineDevice->SetRenderState(D3DRS_INDEXEDVERTEXBLENDENABLE, TRUE);
            pBoneComb = reinterpret_cast<LPD3DXBONECOMBINATION>(pmcMesh->m_pBoneCombinationBuf->GetBufferPointer());
            for (ipattr = 0; ipattr < pmcMesh->cpattr; ipattr++)
            {
                for (DWORD i = 0; i < pmcMesh->m_paletteSize; ++i)
            ...
            mpEngineDevice->SetRenderState(D3DRS_INDEXEDVERTEXBLENDENABLE, FALSE);
            mpEngineDevice->SetRenderState(D3DRS_VERTEXBLEND, 0);
        ...
        else if (m_method == SOFTWARE)
        {
            ...
            hr = mpEngineDevice->SetTransform(D3DTS_WORLD, &Identity);
            if (FAILED(hr))
                return hr;
            ...
            for (ipattr = 0; ipattr < pmcMesh->cpattr; ipattr++)
            {
                mpEngineDevice->SetMaterial(&(pmcMesh->rgMaterials[pmcMesh->m_pAttrTable[ipattr].AttribId]));
                mpEngineDevice->SetTexture(0, pmcMesh->pTextures[pmcMesh->m_pAttrTable[ipattr].AttribId]);
    ...
    else
    {
        for (ipattr = 0; ipattr < pmcMesh->cpattr; ipattr++)
        {
            EngineSetMaterial(&(pmcMesh->rgMaterials[ipattr]));
            mpEngineDevice->SetTexture(0, pmcMesh->pTextures[ipattr]);
		    hr = pmcMesh->pMesh->DrawSubset( ipattr );
            if(FAILED(hr))
                return hr;
        }
...
HRESULT DrawFrames(SFrame *pframeCur, UINT &cTriangles)
{
    ...
    {
        hr = mpEngineDevice->SetTransform(D3DTS_WORLD, &pframeCur->matCombined);
        if(FAILED(hr))
            return hr;
    }
...
void SFrame::SetTime(float fGlobalTime)
{
    ...
    if (m_pMatrixKeys )
    {
        fTime = (float)fmod(fGlobalTime, (float)m_pMatrixKeys[m_cMatrixKeys-1].dwTime);
    ...
    else
    {
        ...
        if (m_pScaleKeys)
        {
            ...
            fTime = (float)fmod(fGlobalTime, (float)m_pScaleKeys[m_cScaleKeys-1].dwTime);
        ...
        if (m_pRotateKeys )
        {
            ...
            fTime = (float)fmod(fGlobalTime, (float)m_pRotateKeys[m_cRotateKeys-1].dwTime);
...
#define USE_SQUAD
        ...
        if (m_pPositionKeys)
        {
            ...
            fTime = (float)fmod(fGlobalTime, (float)m_pPositionKeys[m_cPositionKeys-1].dwTime);
