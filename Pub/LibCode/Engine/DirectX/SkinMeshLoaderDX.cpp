...
SkinnedMeshDX*		mpLoadingSkinMesh = NULL;
                     ...
                     UINT *pcVertices)
{
    ...
    while (pmcCur != NULL)
    {
        ...
        hr = pmcCur->pMesh->LockVertexBuffer(0, (VERTEX_LOCKTYPE)&pbPoints);
        if (FAILED(hr))
            goto e_Exit;
...
HRESULT LoadMeshHierarchy( SkinnedMeshDX* pSkinnedMeshDX, const char* szFilename )
{
	mpLoadingSkinMesh = pSkinnedMeshDX;
	
    TCHAR* pszFile = (TCHAR*)szFilename;
    ...
    {
        hr = LoadFrames(pxofobjCur, pdeMesh, dwOptions, D3DFVF_CUSTOMVERTEX,
            mpEngineDevice,
            pdeMesh->pframeRoot);
    ...
    pdeMesh->pdeNext = mpLoadingSkinMesh->m_pdeHead;
    mpLoadingSkinMesh->m_pdeHead = pdeMesh;
    
    mpLoadingSkinMesh->m_pdeSelected = pdeMesh;
	mpLoadingSkinMesh->m_pmcSelectedMesh = pdeMesh->pframeRoot->pmcMesh;
    
    
    mpLoadingSkinMesh->m_pframeSelected = pdeMesh->pframeRoot;
  ...
  //  SetProjectionMatrix();
    ...
    mpLoadingSkinMesh->m_pdeSelected->fCurTime = 0.0f;
    mpLoadingSkinMesh->m_pdeSelected->fMaxTime = 200.0f;
    
    D3DXMatrixTranslation(&mpLoadingSkinMesh->m_pdeSelected->pframeRoot->matRot,
        -pdeMesh->vCenter.x, -pdeMesh->vCenter.y, -pdeMesh->vCenter.z);
    mpLoadingSkinMesh->m_pdeSelected->pframeRoot->matRotOrig = mpLoadingSkinMesh->m_pdeSelected->pframeRoot->matRot;
...
HRESULT LoadAnimationSet(LPDIRECTXFILEDATA pxofobjCur, SDrawElement *pde,
                                            DWORD options, DWORD fvf, LPGRAPHICSDEVICE pD3DDevice,
                                            SFrame *pframeParent)
{
    SFrame *pframeCur;
    const GUID *type;
                                    ...
                                    SFrame *pframeParent)
{
    ...
    UINT cFaces;
    UINT iMaterial;
    LPDIRECT3DDEVICE8 m_pDevice = mpEngineDevice;
...
HRESULT  DeleteSelectedMesh()
{
    if (mpLoadingSkinMesh->m_pdeSelected != NULL)
    {
        SDrawElement *pdeCur = mpLoadingSkinMesh->m_pdeHead;
        SDrawElement *pdePrev = NULL;
        while ((pdeCur != NULL) && (pdeCur != mpLoadingSkinMesh->m_pdeSelected))
        {
            pdePrev = pdeCur;
        ...
        if (pdePrev == NULL)
        {
            mpLoadingSkinMesh->m_pdeHead = mpLoadingSkinMesh->m_pdeHead->pdeNext;
        }
        else
        {
        ...
        mpLoadingSkinMesh->m_pdeSelected->pdeNext = NULL;
        if (mpLoadingSkinMesh->m_pdeHead == mpLoadingSkinMesh->m_pdeSelected)
            mpLoadingSkinMesh->m_pdeHead = NULL;
        delete mpLoadingSkinMesh->m_pdeSelected;
        mpLoadingSkinMesh->m_pdeSelected = NULL;
    }
