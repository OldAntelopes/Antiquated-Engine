...
int			mnHeightmapPhysicsObject = 0;

//float		mfLandscapeHeightScale = 1.0f;
float		mfLandscapeHeightScale = 0.85f;


void		LandscapeHeightmapSetHeightScale( float fHeightScale )
{
	mfLandscapeHeightScale = 0.85f * fHeightScale;
}
...
int		nMapPos = ( nMapY * mnHeightmapSizeX ) + nMapX;
	...
	{
		return( mpfHeightmap[nMapPos] * mfLandscapeHeightScale );
	}
	return( 0.0f );
}

void		LandscapeHeightmapAddToPhysicsWorld( void )
{
	mnHeightmapPhysicsObject = PhysicsObjectAddHeightfield( mnHeightmapSizeX, mnHeightmapSizeY, mpfHeightmap, LandscapeMapToWorldScale(), mfLandscapeHeightScale );
}
...
//int	nIndex;
	...
	fFractionAlongXLine = ( fWorldX - xBasePos.x ) / (fMapToWorldScale);
	fFractionAlongYLine = ( fWorldY - xBasePos.y ) / (fMapToWorldScale);
...
//		fImageStepY *= -1.0f;
		...
		InterfaceReleaseTexture( hTexture );
