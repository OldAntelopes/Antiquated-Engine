...
int                Rc;
  ...
  Rc = select (1+ListenSock, & ReadMask, NULL, NULL, & TO);
  if (Rc<0)   
  {
  ...
  if (Rc==0)
  {
	...
	if ( FD_ISSET( ListenSock, &ReadMask ) != 0 )
	{
		Tcp4uLog (LOG4U_CALL, "accept on listen socket %d", ListenSock);
		ConSock = accept (ListenSock, (struct sockaddr far *) pAddress, &nAddrLen);
 ...
 *lpPort = htons (saSockAddr.sin_port);
  ...
  if (IsBadWritePtr (pS, sizeof *pS))  
  {
	...
	int		nSize = sizeof( int );
		...
		{
			nVal = 65536;
			setsockopt( ListenSock, SOL_SOCKET, SO_RCVBUF, (char*)(&nVal), nSize );
		}

		getsockopt( ListenSock, SOL_SOCKET, SO_SNDBUF, (char*)(&nVal), &nSize );

		if ( nVal < 65536 )
		{
			nVal = 65536;
			setsockopt( ListenSock, SOL_SOCKET, SO_SNDBUF, (char*)(&nVal), nSize );
		}
	}
