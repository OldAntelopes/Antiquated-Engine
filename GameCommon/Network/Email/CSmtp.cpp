...
void CSmtp::Send()
{
	...
	if(hSocket==INVALID_SOCKET)
	{
		...
		{
			goto error; //			throw ECSmtp(ECSmtp::WSA_INVALID_SOCKET);
		}
	}	
...
//	try
	{
		...
		{
			goto error; //	throw ECSmtp(ECSmtp::LACK_OF_MEMORY);
		}
		...
		for(FileId=0;FileId<Attachments.size();FileId++)
		{
			...
			if(hFile == NULL)
			{
				goto error; // throw ECSmtp(ECSmtp::FILE_NOT_EXIST);
			}
			...
			{
				goto error;//	throw ECSmtp(ECSmtp::MSG_TOO_BIG);
			}
		...
		if(!m_sMailFrom.size())
		{
			goto error;//	throw ECSmtp(ECSmtp::UNDEF_MAIL_FROM);
		}
		...
		{
			goto error;//	throw ECSmtp(ECSmtp::UNDEF_RECIPIENTS);
		}
...
//	try
	{
		...
		if((sockAddr.sin_addr.s_addr = inet_addr(szServer)) == INADDR_NONE)
		{
			...
			else
			{
				...
				closesocket(hSocket);
#endif
				goto error; //throw ECSmtp(ECSmtp::WSA_GETHOSTBY_NAME_ADDR);
			}				
		}
...
#endif
		{
			...
			closesocket(hSocket);
#endif
			goto error; //throw ECSmtp(ECSmtp::WSA_IOCTLSOCKET);
		}
		...
		if(connect(hSocket,(LPSOCKADDR)&sockAddr,sizeof(sockAddr)) == SOCKET_ERROR)
		{
...
#endif
			{
				...
				closesocket(hSocket);
#endif
				goto error;//throw ECSmtp(ECSmtp::WSA_CONNECT);
			}
		}
		else
			return true;
		...
		while(true)
		{
			...
			if((res = select(hSocket,NULL,&fdwrite,&fdexcept,&timeout)) == SOCKET_ERROR)
			{
#ifdef LINUX
				close(hSocket);
#else
				closesocket(hSocket);
#endif
				goto error;//				throw ECSmtp(ECSmtp::WSA_SELECT);
			}
		...
		if(authenticate && IsKeywordSupported(RecvBuf, "AUTH") == true)
		{
			...
			else if(IsKeywordSupported(RecvBuf, "CRAM-MD5") == true)
			{
				...
				if(!ustrChallenge || !ustrPassword)
					goto error;//throw ECSmtp(ECSmtp::BAD_LOGIN_PASSWORD);
			...
			else if(IsKeywordSupported(RecvBuf, "DIGEST-MD5") == true)
			{
				...
				if(find<0)
					goto error;//throw ECSmtp(ECSmtp::BAD_DIGEST_RESPONSE);
				...
				if(!getpeername(hSocket, (struct sockaddr*)&addr, &len))
					goto error;//throw ECSmtp(ECSmtp::BAD_SERVER_NAME);
			...
			{
				goto error;//throw ECSmtp(ECSmtp::LOGIN_NOT_SUPPORTED);
			}
		}
	}
